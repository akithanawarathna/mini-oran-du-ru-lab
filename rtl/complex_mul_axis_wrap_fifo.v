    ...
    /////////////////////////////////////////////////////////////////////////////////////////
        ...
        module complex_mul_axis_wrap_fifo     
        (
            input  wire        aclk,
            input  wire        aresetn,
            ...
            // pipe_stall: stall input when fifo is occupied
            // and downstream is not consuming
            assign pipe_stall = fifo_valid && !m_axis_tready;
            
            //assign fifo_full = (wr_ptr_fifo[4] != rd_ptr_fifo[4]) && (wr_ptr_fifo[3:0] == rd_ptr_fifo[3:0]);
                ...
                if (!aresetn) begin
                    ...
                    if (core_out_valid) begin
                        if (!m_axis_tvalid || m_axis_tready) begin
                            ...
                            // Output port busy - park in fifo
                            //fifo_valid <= 1'b1;
                            fifo_data[wr_ptr_fifo] <= core_result;
                            wr_ptr_fifo <= wr_ptr_fifo + 1'b1;
                            if ( (wr_ptr_fifo[4] != rd_ptr_fifo[4]) && (wr_ptr_fifo[3:0] == rd_ptr_fifo[3:0]) )
                                fifo_valid <= 1'b1;
                            fifo_last[wr_ptr_fifo]  <= tlast_d3;
                        end
                    end
                    ...
                    if (m_axis_tready && m_axis_tvalid) begin
                        if (fifo_valid) begin
                            // Drain fifo into output
                            //m_axis_tdata  <= fifo_data;
                            m_axis_tdata  <= fifo_data[rd_ptr_fifo];
                            m_axis_tlast  <= fifo_last[rd_ptr_fifo];
                            rd_ptr_fifo <= rd_ptr_fifo + 1'b1;
                            m_axis_tvalid <= 1'b1;
                            if(wr_ptr_fifo == rd_ptr_fifo)
                                fifo_valid    <= 1'b0;
