...
parameter integer PIPE_LAT   = 3;
    ...
    // -----------------------------------------------------------------------
    //  Task : send_beat
    //  Drives tvalid+tdata+tlast and holds until handshake.
    //  After handshake, drops tvalid and inserts ONE idle cycle.
    // -----------------------------------------------------------------------
    task send_beat;
        input [63:0] data;
        input        last;
        begin
            ...
            // Wait until handshake occurs on a rising edge
            while (s_axis_tready === 1'b0)
                @(posedge aclk);
            @(posedge aclk); // handshake edge (tvalid=1 & tready=1)
    ...
    // -----------------------------------------------------------------------
    //  Task : receive_beat  (FIXED)
    //  Assert tready, wait until tvalid=1.
    //  Sample tdata/tlast BEFORE the pop edge.
    //  Then allow one clock edge for the transfer (pop), drop tready.
    // -----------------------------------------------------------------------
    task receive_beat;
        output [31:0] data;
        output        last;
        begin
            ...
            // Wait until a cycle where VALID is high
            while (m_axis_tvalid === 1'b0)
                @(posedge aclk);

            // Sample in same cycle before the pop edge
            #1;
            data = m_axis_tdata;
            last = m_axis_tlast;

            // Now consume it on next rising edge
            @(posedge aclk);
            m_axis_tready = 1'b0;
    ...
    initial begin
        ...
        // ==================================================================
        //  TEST 2 : Single Transaction  (FIXED)
        //  Use receive_beat instead of sampling tvalid at a fixed time.
        // ==================================================================
        $display("\n--- Test 2 : Single Transaction ---");
        beat_data = 64'hAABBCCDD11223344;
        beat_last = 1'b1;
        ...
        receive_beat(rx_data, rx_last);
        check(rx_last === 1'b1, "TLAST=1 for single-beat packet");
        ...
        receive_beat(rx_data, rx_last); check(rx_last === 1'b0, "BB Beat 1 : tlast=0");
        receive_beat(rx_data, rx_last); check(rx_last === 1'b0, "BB Beat 2 : tlast=0");
        receive_beat(rx_data, rx_last); check(rx_last === 1'b0, "BB Beat 3 : tlast=0");
        receive_beat(rx_data, rx_last); check(rx_last === 1'b1, "BB Beat 4 : tlast=1");
        ...
        // With backpressure, FIFO should become non-empty at some point
        wait_cycles(PIPE_LAT + 4);
        check(m_axis_tvalid === 1'b1, "FIFO non-empty under backpressure");

        // Now drain
        m_axis_tready = 1'b1;
        receive_beat(rx_data, rx_last); check(rx_last === 1'b0, "BP Beat 1 : tlast=0");
        receive_beat(rx_data, rx_last); check(rx_last === 1'b0, "BP Beat 2 : tlast=0");
        receive_beat(rx_data, rx_last); check(rx_last === 1'b0, "BP Beat 3 : tlast=0");
        receive_beat(rx_data, rx_last); check(rx_last === 1'b1, "BP Beat 4 : tlast=1");
        m_axis_tready = 1'b0;

        wait_cycles(2);

        // ==================================================================
        //  TEST 5 : FIFO Overflow Protection (FIXED EXPECTATION)
        //  With pipeline latency, upstream may accept up to FIFO_DEPTH + PIPE_LAT
        //  beats before tready deasserts while downstream is stalled.
        // ==================================================================
        $display("\n--- Test 5 : FIFO Overflow Protection ---");
        apply_reset;

        m_axis_tready = 1'b0; // stall downstream
        accepted = 0;

        for (k = 0; k < FIFO_DEPTH + PIPE_LAT + 2; k = k + 1) begin
            s_axis_tvalid = 1'b1;
            s_axis_tdata  = {16'hABCD, k[15:0], 16'h1234, k[15:0]};
            s_axis_tlast  = (k == (FIFO_DEPTH + PIPE_LAT + 1)) ? 1'b1 : 1'b0;
            @(posedge aclk);
            if (s_axis_tready === 1'b1)
                accepted = accepted + 1;
        end
        ...
        wait_cycles(PIPE_LAT + 4);
        check((accepted <= (FIFO_DEPTH + PIPE_LAT)),
              "tready throttled: accepted <= FIFO_DEPTH + PIPE_LAT");

        $display("    Accepted %0d of %0d attempted (FIFO_DEPTH=%0d, PIPE_LAT=%0d)",
                  accepted, (FIFO_DEPTH + PIPE_LAT + 2), FIFO_DEPTH, PIPE_LAT);

        // Drain FIFO
        m_axis_tready = 1'b1;
        wait_cycles(FIFO_DEPTH + PIPE_LAT + 6);
        m_axis_tready = 1'b0;
